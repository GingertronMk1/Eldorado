...
myMaximumBy' currMax currMaxVal f (a:as) = let newMaxVal = f a
...
allOptionsProcessed = map (
    map (\(players, team) -> (head team, players))
    . sortOn (Down . length . snd)
    . map unzip
    . groupBy (\(_, t1) (_, t2) -> t1 == t2) . sortOn snd
    )
  . allOptions
...
ppIteration (iterationCount, maxValue, options) = intercalate "\n" $ ("Iteration " ++ show iterationCount):map (("  "++) . (\(team, players) -> team ++ ": " ++ intercalate ", " players)) options
